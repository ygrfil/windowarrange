...
- The main panel uses a top-aligned workspace board. Its single toolbar owns Auto, the adjacent **RnG** overlay toggle, Arrange, the **Space** toggle for Preserve table slots, Settings, and tray hiding, with Space between Arrange and Settings; the poker pane is a uniformly scaled mirror of the selected work area with active slots, muted numbered Placeholders tiles, named interactive parked-table miniatures at their actual positions, no reserved parked-table ghosts, and one combined lobby-count card immediately below the mirror. The right pane owns compact ordinary-window cards. An independent native Settings viewport owns display selection, the icon legend, the ordinary-window default, counts/status, and hotkey details.
