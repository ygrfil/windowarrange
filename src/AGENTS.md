...
- The main panel uses a top-aligned unified workspace board. Its toolbar order is **Arrange, Space, Auto, RnG, Settings, GGLobby**, one detected-client-icon button per parked poker table, then tray hiding; Auto and RnG use name-only labels with selected color for state, and GGLobby is dark orange. Parked-table buttons locate on primary click and unpark on secondary click. One full-width uniformly scaled mirror of the selected work area shows active slots and muted numbered Placeholders tiles, but no parked-table miniatures or reserved parked-table ghosts. Mode-sorted ordinary-window tiles fill only the full-height strip right of the projected edge of every active slot, placeholder, and preserved-space slot. The application grid uses one full strip tile for one window, a two-high column for two, a two-by-two grid for four, and a dense column-major grid for larger counts. Each ordinary tile caches and centers the real Windows icon obtained from its top-level window, scaled within the free center without covering its title or controls. A subtle divider attaches a responsive dock containing only ignored-poker chips. GGLobby sequentially locates permanently parked lobbies. An independent native Settings viewport owns display selection, the icon legend, the ordinary-window default, counts/status, and hotkey details.
...
- The main viewport is non-maximizable, 680–820 points wide, and automatically fits between 180 and 620 points high from its actual width, selected-work-area aspect ratio, and visible dock rows, with a 740×480 initial size. Height calculations must include toolbar, separators, frame margins, row gaps, and a minimal DPI-safe bottom allowance so the final mirror or dock row is fully visible without restoring a large unused band. The root frame fills all available space. The main workspace has no scroll areas, and no dock chip may expand its fixed cell.
...
- ClubGG and LDPlayer tiles expose icon controls for **Active**, **Park**, and **Ignore**; primary-clicking the tile body performs Locate, while only its enlarged numbered-badge hit target participates in swap selection. Ordinary application mirror tiles expose **Ignore**, **Fill space**, and **Top-right** controls and locate on body click. GGLobby invokes the ordinary single-window Locate path for each lobby sequentially with a short gap, raising each permanently parked lobby at its existing position without resizing or repositioning it. With Preserve table slots enabled, parking releases only that poker slot into an anonymous placeholder so every unaffected active table retains its column and row; its toolbar client-icon button locates on primary click and unparks it into the earliest compatible available assignment.
- Configuration schema version 8 persists table, parked, ignored, top-right, and free-space dispositions; typed poker columns; explicit anonymous closed-table placeholders; the default-on Preserve table slots behavior; legacy order for migration; and the configurable ordinary-window default. Missing fields deserialize safely, and the version-6 `reserve_two_slots` key aliases to the new setting. Ordinary-window choices write exact plus process/class fallback rules, while poker-table choices use exact signatures only. Remove legacy poker process/class fallbacks during reconciliation. Restore saved poker Park rules only on initial startup discovery and for one matching live occurrence; after initial discovery, every newly appearing poker window ID starts Active even if a stale exact Park signature matches.
...
- Explicit Arrange and automatic reflow must reuse persisted slot ownership. With Preserve table slots enabled, neither path may compact, renumber, swap, resize, or offset unaffected live tables after another table is parked, ignored, closed, discovered, or manually moved. Preserve the vacated column's ClubGG/LDPlayer geometry; only a layout-size reduction required to fit the selected work area may resize existing slot geometry.
- Global hotkeys cover Arrange, Show/hide panel, and GGLobby only; never expose or register focused-table or numbered-table shortcuts. The panel hotkey hides a currently visible panel and restores, shows, and focuses a hidden or minimized panel centered around the mouse pointer and clamped inside that monitor's work area. The GGLobby hotkey invokes the same sequential Locate command as the toolbar button. Tray activation remains a one-way Show action and does not reposition the panel.
...
- Do not manipulate internal ClubGG or LDPlayer child controls; only independently movable eligible windows are managed. Classify untitled, exact-title, and explicitly lobby-titled ClubGG top-level surfaces as ancillary **ClubGG lobby** candidates. Always resolve every lobby disposition to Park regardless of saved rules or mode commands, expose GGLobby sequential Locate through the toolbar and configurable shortcut, and never assign lobbies a poker column, half-slot, table number, per-table shortcut, preserved placeholder, parked ghost, or ordinary-window tile. Match LDPlayer only through the `dnplayer.exe` frontend, never its headless or service processes.
...
- `cargo fmt --check`
- For small dependency-neutral features, use `cargo clippy --bin table-arranger-control --all-features -- -D warnings` and skip tests unless requested or warranted by risk.
- For broader or high-risk changes, use `cargo clippy --all-targets --all-features -- -D warnings` and `cargo test --all-targets`.
