...
- Explicit Arrange and automatic reflow must retain each unaffected live poker table's persisted column, row, size, and screen position when another table is ignored, discovered, or manually rearranged. Parking and closing release that table without creating a placeholder, remove newly unused trailing/whole-column space, and may shift later compatible tables forward without exchanging their relative order. Resize existing slot geometry only when required to fit the selected work area.
