...
- Park disabled poker tables and ClubGG lobbies at their actual minimum supported sizes, starting at the selected display's bottom-right and continuing directly left without overlap; wrap upward only when a row cannot fit.
...
- Keep every ClubGG lobby visible in a separate right-side **ClubGG lobbies** UI group, default it to Park, and exclude it from poker geometry, numbering, hotkeys, ghosts, and preservation calculations.
