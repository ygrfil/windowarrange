...
- Present a compact unified workspace board: a single top toolbar with Auto, RnG, Arrange, Space, Settings, and tray hiding; one full-width uniformly scaled spatial mirror of poker slots and actual parked positions; and an attached responsive six-to-eight-column dock of compact chips for the combined ClubGG lobby, ordinary application windows, and ignored poker windows. Use compact painter-drawn action icons throughout. Actual parked miniatures show the table name, locate on left-click, and unpark on right-click. Put display selection, the complete icon legend, default ordinary-window behavior, counts/status, and hotkey details in an independent native Settings window.
- Keep the panel responsive within 680–820 points wide and automatically fit its height between 180 and 620 points to the full-width mirror and visible dock rows, with a 740×480 initial size. Disable maximizing, fill the full viewport background, and show the complete main workspace without scroll areas. Constrain every chip to its dock cell.
...
- Every pixel of a poker-table tile, ignored-poker chip, or ordinary-window dock chip performs Locate on primary click except the numbered swap badge and the exact action-button rectangles. Compute explicit non-overlapping body hit regions by subtracting those controls so labels, margins, borders, and gaps respond consistently. Use a direct button response for the combined lobby chip and locate its detected windows sequentially with a short gap, using the same single-window Locate backend call. Lobby Locate must only raise each window at its current parked position and must never resize or reposition it. Clicking a poker tile's enlarged numbered-badge hit target selects it for swapping, then clicking another occupied number swaps individual same-client tables or whole ClubGG/LDPlayer columns, while clicking an empty placeholder moves the selected table and preserves the old hole. Persist spatial assignments and cancel selection when the same numbered badge is clicked again.
...
- Keep the panel compact and modern: a short horizontal toolbar, one full-width desktop-proportional poker mirror with labeled placeholders and interactive named parked miniatures at their actual positions but no reserved parked-table ghosts, plus one attached compact dock for lobbies, ordinary windows, and ignored poker windows with matching vector controls; keep all secondary controls/status in Settings.
...
- Keep the workspace visually structured and space-efficient: start the full-width monitor mirror directly below the toolbar, attach the compact window dock with only a subtle divider, omit section descriptions, and automatically shrink panel height when fewer dock rows are visible without clipping the final row.
