...
- Present a compact unified workspace board with toolbar actions ordered **Arrange, Space, Auto, RnG, Settings, GGLobby**, followed by tray hiding. Auto and RnG show only their names and communicate On/Off through selected color. Render GGLobby dark orange. Use one full-width uniformly scaled spatial mirror of poker slots and actual parked positions. Fill only the full-height mirror strip to the right of all active poker slots, placeholders, and preserved-space slots with ordinary application tiles: one window fills the strip, two split one column into equal halves, four form a two-by-two grid, and larger counts continue in a dense column-major grid. Center each ordinary window's real Windows application icon in its tile without covering its title or action controls. Keep only ignored-poker chips in the attached responsive dock. Use compact painter-drawn action icons throughout. Actual parked poker miniatures show the table name, locate on left-click, and unpark on right-click. Put display selection, the complete icon legend, default ordinary-window behavior, counts/status, and hotkey details in an independent native Settings window.
...
- Keep the panel compact and modern: a short horizontal toolbar ordered Arrange, Space, Auto, RnG, Settings, GGLobby with color-only toggle state and a dark-orange GGLobby button; one full-width desktop-proportional poker mirror with labeled placeholders and interactive named parked miniatures at their actual positions but no reserved parked-table ghosts; ordinary-window tiles filling only the unused full-height right strip; and one attached compact dock only for ignored poker windows. Keep all secondary controls/status in Settings.
