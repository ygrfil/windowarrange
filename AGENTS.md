...
- Default ClubGG and LDPlayer poker tables first discovered after application startup to **Arrange**, even while another table is parked and even when a stale exact Park signature matches; saved Park restoration applies only during initial startup discovery and to one matching live occurrence. The configurable ordinary-window default starts at **Ignore** and may be changed in Settings to **Top-right** or **Fill space**; an explicit saved window rule always overrides the default.
...
- Provide a persistent, always-clickable **Space** toolbar button for the **Preserve table slots** preference, enabled by default. Place it between Arrange and Settings. Baseline anonymous reserved space has a maximum four-table/two-column total footprint, not extra columns beyond that footprint. Parking and closing never create placeholders; they release the vacated slot and newly unused geometry. Explicit holes from Ignore or manual placeholder moves remain persisted. LDPlayer must consume an available baseline reserved column. When more than four poker tables are unparked, suppress unused baseline and trailing placeholder-only reservation without overwriting the saved preference or intentional holes, but retain internal geometry required to preserve live-table relative order. A manual Off preference remains off.
...
- Explicit Arrange and automatic reflow must retain each unaffected live poker table's persisted column, row, size, and screen position when another table is ignored, discovered, or manually rearranged. Parking and closing release that table without creating a placeholder, remove only newly unused trailing space, and must keep remaining live tables in their existing slots and screen rectangles. Resize existing slot geometry only when required to fit the selected work area.
...
- Present a compact unified workspace board with toolbar actions ordered **Arrange, Space, Auto, RnG, Settings, GGLobby**, followed by one client-icon button per parked poker table and tray hiding. Auto and RnG show only their names and communicate On/Off through selected color. Render GGLobby dark orange. A parked-table button uses the detected ClubGG or Pokerrr 2 application icon, locates its table on left-click, and unparks it on right-click. Use one full-width uniformly scaled spatial mirror of poker slots without parked-table miniatures. Fill only the full-height mirror strip to the right of all active poker slots, placeholders, and preserved-space slots with ordinary application tiles: one window fills the strip, two split one column into equal halves, four form a two-by-two grid, and larger counts continue in a dense column-major grid. Center each ordinary window's real Windows application icon in its tile without covering its title or action controls. Keep only ignored-poker chips in the attached responsive dock. Use compact painter-drawn action icons throughout. Put display selection, the complete icon legend, default ordinary-window behavior, counts/status, and hotkey details in an independent native Settings window.
- Keep the panel responsive within 680–820 points wide and automatically fit its height between 180 and 620 points to the full-width mirror and visible ignored-poker dock rows, with a 740×480 initial size. Disable maximizing, fill the full viewport background, and show the complete main workspace without scroll areas. Constrain every mirror tile and dock chip to its assigned rectangle.
...
- Every pixel of a poker-table tile outside its exact action-button rectangles participates in two-step move/swap selection; the numbered badge is part of that same tile-wide hit area. Clicking another occupied tile swaps individual same-client tables or whole ClubGG/LDPlayer columns, while clicking an empty placeholder moves the selected table and preserves the old hole. Persist spatial assignments and cancel selection when the selected tile is clicked again. Every pixel of an ignored-poker chip or ordinary-window mirror tile performs Locate on primary click except its exact action-button rectangles. Compute explicit non-overlapping hit regions by subtracting those controls so labels, margins, borders, and gaps respond consistently. The toolbar GGLobby button locates detected lobbies sequentially with a short gap, using the same single-window Locate backend call. Lobby Locate must only raise each permanently parked window at its current position and must never resize or reposition it.
...
- Reconcile user-driven screen moves back into the persisted poker-slot map so the spatial mirror follows manual reordering without immediately moving the windows again. When a live table closes, release its identity and space without retaining a placeholder; a newly opened table uses the earliest compatible available slot in numeric order rather than reclaiming the closed table's former position by identity.
...
- The verified release baseline is Rust/Cargo `1.98.0` with `windows 0.62.2`, `eframe 0.36.1`, `tray-icon 0.24.2`, `global-hotkey 0.8.0`, `serde 1.0.229`, `serde_json 1.0.151`, `thiserror 2.0.20`, `log 0.4.34`, `crossbeam-channel 0.5.16`, and development dependency `proptest 1.11.0`.
...
- `Build-Release.cmd` is the one-click release entry point and delegates to `build-release.ps1`. The script advances the patch version only when its archive already exists, updates `Cargo.toml` and the application entry in `Cargo.lock`, runs the small-change verification without tests, publishes both executable names, and verifies their version metadata and SHA-256 equality.
...
- Keep spatial assignments stable while the saved Preserve table slots preference is on. Ignore or a manual placeholder move leaves a persisted anonymous placeholder at the vacated screen slot. Parking and closing never create placeholders: they release that space and must keep remaining live tables in their existing slots and screen rectangles. A parked table remains accessible only through its toolbar client-icon button. Unparked and newly opened tables use the earliest compatible available slot in numeric order and do not reclaim a former identity-owned position. Baseline anonymous reservation must not expand beyond a four-table/two-column footprint. Let LDPlayer claim an available baseline reserved column. With more than four unparked tables, suppress unused baseline/trailing reservation but retain unrelated intentional holes. Manual Off compacts only because the user explicitly requested it.
...
- Add new tables as enabled at the end of the current order unless they consume an already empty compatible slot; never insert a new table in a way that changes any existing live table's relative order or undoes a manual mixed-client swap.
- Never let a parked poker table's saved rule or process/class fallback cause a newly opened table to start Parked.
...
- Keep the panel compact and modern: a short toolbar ordered Arrange, Space, Auto, RnG, Settings, GGLobby, then one detected-client-icon button for every parked poker table, with color-only toggle state and a dark-orange GGLobby button; one full-width desktop-proportional poker mirror with labeled placeholders and no parked-table miniatures or reserved parked-table ghosts; ordinary-window tiles filling only the unused full-height right strip; and one attached compact dock only for ignored poker windows. Parked-table buttons locate on left-click and unpark on right-click. Keep all secondary controls/status in Settings.
...
- Keep the workspace visually structured and space-efficient: start the full-width monitor mirror directly below the toolbar, divide its unused right-side strip among ordinary windows according to the detected count without covering poker slots or reserved placeholders, attach the ignored-poker dock with only a subtle divider, omit section descriptions, and automatically shrink panel height to end beneath the final mirror or dock row with only the minimal DPI-safe bottom allowance needed to prevent clipping.
...
- Preserve up to a four-table/two-column total poker footprint for Fill-space applications and mirrored Placeholders by default; LDPlayer consumes an available reserved column. Above four unparked poker tables, suppress unused baseline/trailing reservation without moving live tables to close internal holes. Manual Off plus spatial assignments survive restart.
...
- Parking, closing, and unparking must retain all continuously active tables in their existing slots with Space either On or Off. Unpark into the earliest compatible free slot, including repeated same-name tables. Space Off removes baseline reservation; compact existing assignments only when the user explicitly switches Space off, never on subsequent automatic reflows.
