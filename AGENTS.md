...
- Provide a persistent, always-clickable **Space** toolbar button for the **Preserve table slots** preference, enabled by default. Place it between Arrange and Settings. Baseline anonymous reserved space has a maximum four-table/two-column total footprint, not extra columns beyond that footprint; explicit holes from formerly occupied closed-table slots are retained in persistence. LDPlayer must consume an available baseline reserved column. When more than four poker tables are unparked, omit all reserved and placeholder-only geometry without overwriting the saved preference or holes; restore it at four or fewer. A manual Off preference remains off.
...
- Explicit Arrange and automatic reflow must retain each unaffected live poker table's persisted column, row, size, and screen position when another table is parked, ignored, closed, discovered, or manually rearranged. Do not compact, renumber, or change a vacated column's ClubGG/LDPlayer geometry while Preserve table slots is enabled; resize existing slot geometry only when required to fit the selected work area.
...
- Keep spatial assignments stable while the saved Preserve table slots preference is on and at most four poker tables are unparked. Closing, ignoring, or parking releases identity ownership but leaves a persisted anonymous placeholder at the vacated screen slot, so unaffected active tables retain their existing columns and rows; Park appears only as its toolbar client-icon button. Baseline anonymous reservation must not expand beyond a four-table/two-column footprint. Let LDPlayer claim an available baseline reserved column. With more than four unparked tables, show and arrange active assignments only, omitting placeholder-only geometry until the active count returns to four or fewer; manual Off compacts persistently.
...
- Preserve up to a four-table/two-column total poker footprint for Fill-space applications and mirrored Placeholders by default; LDPlayer consumes an available reserved column. Preservation is inactive above four unparked poker tables and omits all placeholder-only geometry at that count, while manual Off plus spatial assignments survive restart.
