...
- Controller tests must verify that parked tables release their slot ownership, compact the remaining active assignments, appear only as actual-position miniatures, and—together with parked lobbies—use queried minimum sizes and line up from bottom-right toward the left without overlap.
