...
- Controller tests must verify that Preserve table slots defaults on, never adds baseline anonymous geometry beyond a four-table/two-column footprint, lets LDPlayer claim a reserved column, repairs the legacy middle-gap form, suppresses unused baseline/trailing reservation above four unparked tables without moving any live table to close an internal hole or undoing a manual mixed-client swap, appends newly discovered tables without displacing that mixed order when no compatible hole exists, remains manually editable, never overrides manual Off, and compacts immediately while Auto is off only when manually disabled.
...
- Controller tests must verify that parking creates no placeholder, releases newly unused geometry without exchanging remaining live-table order, and unparking consumes the earliest compatible existing placeholder rather than reclaiming its former slot. Parked tables appear only as toolbar client-icon buttons, use queried minimum sizes, and line up from top-right toward the left without overlap. A parked poker rule must never propagate through a process/class fallback to a newly opened table, a stale exact Park signature must not park a poker window first seen after startup, and one exact Park rule may claim only one matching live startup occurrence. Parked lobbies must retain their bottom-right-to-left placement.
- Controller tests must verify that native screen moves update and persist UI slot order without immediate reflow; closing a table creates no placeholder, releases newly unused geometry without exchanging remaining relative order, and does not retain identity ownership for a subsequently opened table.
