...
- Manual UI checks must cover automatic panel-height fitting, full background painting, toolbar order Arrange/Space/Auto/RnG/Settings/GGLobby with name-only toggle labels and dark-orange GGLobby, a desktop-proportional poker mirror with active tiles, muted Placeholders badges, no parked-table ghosts, named interactive parked miniatures at their actual positions, the ordinary-window grid filling only the unused full-height right strip, 1–8 tables plus ordinary-window choices visible without scrolling, the ignored-poker dock, the separate Settings window, and shared painter-drawn icon controls.
