...
- Controller tests must verify that Preserve table slots defaults on, never adds baseline anonymous geometry beyond a four-table/two-column footprint, lets LDPlayer claim a reserved column, repairs the legacy middle-gap form, automatically suppresses above four unparked tables and omits placeholder-only geometry then, remains manually editable, never overrides manual Off, and compacts immediately while Auto is off when manually disabled.
