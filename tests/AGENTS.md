...
- Manual UI checks must cover automatic panel-height fitting, full background painting, visible toolbar actions, a desktop-proportional poker mirror with active tiles, muted Placeholders badges, parked ghosts and named interactive parked miniatures, one lobby-count card directly below the mirror, bounded poker/application panes, 1–8 tables plus ordinary-window choices visible without scrolling, the separate Settings window, and shared painter-drawn icon controls.
